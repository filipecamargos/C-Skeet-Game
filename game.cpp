...
Bird* Game :: createBird()
{
   ...
   //This code will help to generate different points to start To make the game more challeging
   Point point;
   int startPoint = rand() % 200 + 1; // Generate a Point for Y
   int selecSign = rand() % 10 + 1; // Determin if it is negative or Positive (bellow or above 0)
   if(selecSign <= 5)
   {
	   startPoint *= -1;
   }
   point.setX(-200); // send the codinate for X always fot the left side;
   point.setY(startPoint); // Send teh Cordinate for Y

   //Determine the type of Bird
   int typeBird = 0;
   typeBird = rand() % 9 + 1; // This will give a bigger range since rand() follows pattern

   if (typeBird <= 3)
   {
	   newBird = new Standard(point);
   }

   else if (typeBird >= 3 && typeBird <= 6)
   {
	   newBird = new Sacred(point);
   }

   else if (typeBird >= 6 && typeBird <= 9)
   {
	   point.setY(200); //This make the top bird always shows up in the top
	   point.setX(startPoint); //Set a ramdom condition for the x cordinates

	   newBird = new Tough(point); // Creat  our Tough bird
   }
